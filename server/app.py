...
class MemberOnlyIndex(Resource):
    def get(self):
        ...
        if user_id:
            # Filter and return only member-only articles
            member_articles = [
                article.to_dict()
                for article in Article.query.filter(Article.is_member_only == True)
            ]
            response = make_response(jsonify(member_articles), 200)
            return response
        else:
...
class MemberOnlyArticle(Resource):
    def get(self, id):
        user_id = session.get("user_id")
        if user_id:
            members_article_by_id = Article.query.filter(
                Article.is_member_only == True, Article.id == id
            ).first()

            dict = members_article_by_id.to_dict()
